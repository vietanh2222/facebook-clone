...
function Post({
  ...
  useEffect(() => {
    return () => {
      ...
      window.removeEventListener("click", handleCloseChangeComment);
  ...
  const handleRemove = async () => {
    await userComment.forEach((commentUser) =>
      handleRemoveComment(commentUser.id)
    );
  ...
  const handleRemoveComment = async (commentId) => {
    ...
    const reactionListOfComment =
      (await userReactionComment.filter(
        (comment) => comment.commentId === commentId
      )) || [];
    await reactionListOfComment.forEach((reaction) =>
      deleteDoc(doc(db, "userReactionComment", reaction.id))
    );
    const imageNameDelete = await userComment.find(
  ...
  const handleToggleChangeOptions = () => {
    if (!isChangeOptionsOpen) {
      setIsChangeOptionsOpen(true);
      handleCloseChangeComment();
      window.addEventListener("click", handleCloseChangeOptions);
    } else {
      window.removeEventListener("click", handleCloseChangeOptions);
      setIsChangeOptionsOpen(false);
    }
  ...
  const handleToggleChangeComment = (index) => {
    if (isChangeCommentOpen === index) {
      ...
      window.addEventListener("click", handleCloseChangeComment);
    }
  };
    ...
    <div className="post">
      ...
      <div
        className={
          ...
          listUserShare.current.length > 0 ||
      ...
      >
        ...
        <div className="wrapOptions">
          <div
            className="option__count comment__count"
              ...
              {listUserShare.current.map((user, index) => {
                if (index < 8) {
                  ...
                  if (index === 8) {
                    return (
                      <p key={index}>
                        and {listUserShare.current.length - index} others...
                      </p>
                    );
                  } else {
                    return <></>;
      ...
      <div className="post__options">
        ...
        <div
          className={
            isOpenComment ? "option--active post__option" : "post__option"
          }
          onClick={handleOpenComment}
        >
        ...
        <div
          ...
          className={
            isInteract ? "option--active post__option" : "post__option"
          }
        >
          <AccountCircleIcon />
          <ExpandMoreIcon />
        ...
        <div className="post__comments">
                    ...
                    <>
                      <div className="wrapComment">
                        <div className="comment__content">
                          <h2>{comment.user}</h2>
                          <p>{comment.title}</p>
                          {![undefined, ""].includes(comment.image) && (
                        ...
                        <div className="comment__reaction">
                          ...
                          <p>
                            {comment.timestamp && (
                              <Timestamp
                                relative
                                date={comment.timestamp.seconds}
                                autoUpdate
                              />
                            )}
                          </p>
                      ...
                      <div className="comment__reaction-counter">
                        <ReactionCounter
                          ...
                          currentUserReaction={userReactionComment.filter(
                            (commentReaction) =>
                              commentReaction.commentId === comment.id &&
                              commentReaction.user === userLogin
                          )}
                          isComment={true}
                        />
                      </div>
                      <div
                        className="comment__change"
                        onClick={(e) => {
                          e.stopPropagation();
                        }}
                      >
                        <MoreHorizIcon
                          onClick={() => handleToggleChangeComment(index)}
                        />
                        ...
                        <CommentOptions
                          ...
                          commentId={comment.id}
                          handleModifyComment={() =>
                            handleOpenModifyComment(comment.id)
                          }
          ...
          {seeMoreComment ? (
            ...
            userComment.length > 2 && (
              <p onClick={() => setSeeMoreComment(true)}>
                See more {userComment.length - 2}{" "}
                {userComment.length - 2 >= 2 ? "comments" : "comment"}...
              </p>
            )
          )}
          <form>
            ...
            <button
              ...
              disabled={
                (imagePreviewUrl !== "" && imageUpLoadUrl === "") ||
                (comment === "" && imageUpLoadUrl === "")
                  ? true
                  : false
              }
              className={
